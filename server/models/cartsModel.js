...
export const getCarts = (result) => {
    db.query("select 'services' as cart_name, cs.cart_id, cs.cart_service_id as cart_product_id, cs.cart_quantity, s.service_id as product_id, s.name, s.description, s.price, s.reduction, s.image from carts RIGHT JOIN carts_service cs on carts.cart_id = cs.cart_id LEFT JOIN services s on cs.service_id = s.service_id WHERE carts.customer_id = ? UNION select 'goods' as cart_name, cg.cart_id, cg.cart_good_id as cart_product_id, cg.cart_quantity, g.name, g.description, g.price, g.reduction, g.image from carts RIGHT JOIN carts_good cg on carts.cart_id = cg.cart_id LEFT JOIN goods g on cg.good_id = g.good_id WHERE carts.customer_id = ?", (err, results) => {
        if(err) {
