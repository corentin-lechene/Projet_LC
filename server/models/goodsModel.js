...
export const getGoodsOrServicesByCartId = (customer_id, cart_id, result) => {
    db.query("select carts.cart_id,\n" +
        ...
        "  AND customer_id = ?\n" +
        "  AND carts.cart_id = ?\n" +
        "GROUP BY carts.cart_id, carts.customer_id, carts.status, 'goods', cg.cart_good_id, cg.cart_quantity, g.good_id, g.name, g.description, g.price, g.stripe_price, g.reduction, g.image", [customer_id, cart_id, customer_id, cart_id], (err, results) => {
        if (err) {
            result({error: true, reason: err});
        } else {
            result({valid: true, result: results});
    ...
    if (image.ext !== ('jpeg' && 'png' && 'jpg')) {
        ...
        db.query("INSERT INTO goods(name, description, price, seller_id) value(?, ?, ?, ?)", [data.nameGood, data.description, data.price, data.sellers], (err, resultsInsert) => {
            if (err) {
                ...
                const image_name = "img-" + data.sellers + "-" + good_id + "." + image.ext;
